"""
Training script for MAE3D with Channel Cross-Attention on OpenCell dataset.

This script trains a 3D Masked Autoencoder where each channel (nucleus, protein)
is processed as a separate token stream with cross-attention between channels.
...
Usage:
    # Single GPU
    python src/train_mae3d_cross_attention_opencell.py
    ...
    torchrun --nproc_per_node=4 src/train_mae3d_cross_attention_opencell.py
"""
...
from lib.trainers import MAE3DChannelCrossAttentionTrainer
...
def main():
    ...
    parser = argparse.ArgumentParser(description='MAE3D Cross-Attention Training')
    ...
    if cmd_args.config is not None:
        ...
        config_path = os.path.join(code_base_path, 'configs/opencell/opencell_3d_cross_attention.yaml')
    ...
    if rank == 0:
        ...
        print(f'Model: MAE3D with Channel Cross-Attention')
    ...
    trainer = MAE3DChannelCrossAttentionTrainer(args)
