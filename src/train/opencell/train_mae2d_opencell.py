...
def main():
    ...
    # Load config (repo root is four levels up: src/train/opencell/<file>)
    import argparse
    repo_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    parser = argparse.ArgumentParser(description='Train OpenCell MAE2D')
    parser.add_argument('--config', type=str,
                        default=os.path.join(repo_root, 'configs/opencell/opencell_2d.yaml'),
                        help='Path to config file')
    args_cmd, _ = parser.parse_known_args()
    config_path = args_cmd.config
