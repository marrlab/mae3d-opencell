#!/usr/bin/env python3
"""
Training script for OpenCell protein localization classification.
Supports both 2D and 3D models with pretrained MAE weights.

Usage:
    # Train 3D model
    python src/train_localization.py configs/opencell_localization_3d.yaml

    # Train 2D model
    python src/train_localization.py configs/opencell_localization_2d.yaml

    # Multi-GPU training with torchrun
    torchrun --nproc_per_node=4 src/train_localization.py configs/opencell_localization_3d.yaml
"""
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train OpenCell Localization Classifier')
    ...
    parser.add_argument('--mae_embedding_csv_path', type=str, default=None,
                        help='Directory containing the CSVs used during embedding extraction '
                             '(e.g. dataset1/). Required when the localization csv_path differs '
                             'in size from the extraction CSV; enables image_path-based lookup.')
    ...
    # Create trainer
    trainer = LocalizationTrainer(args)
