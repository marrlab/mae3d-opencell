"""
Extract embeddings from MAE3D model and visualize with UMAP.

This script:
1. Loads a trained MAE3D model
...
from lib.models.mae3d import MAE3D, patchify_image, build_3d_sincos_position_embedding
...
def build_mae3d_model(args):
    """Build MAE3D model from config."""
...
def extract_embeddings(model, dataloader, device, pooling='mean'):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Extracting embeddings"):
            ...
            # Transforms produce [B, C, Z, Y, X] = [B, 2, 100, 176, 176]
            # MAE3D expects [B, C, H, W, D] where input_size=[100, 176, 176]
            # So H=100 (Z), W=176 (Y), D=176 (X) - already in correct order!
...
def plot_umap(embeddings, labels, output_path, n_neighbors=15, min_dist=0.1, metric='cosine'):
    ...
    ax.set_title('UMAP of MAE3D Embeddings by Localization', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(description='Extract MAE3D embeddings and visualize with UMAP')
    ...
    # Build model
    model = build_mae3d_model(config)
