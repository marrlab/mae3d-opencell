...
def build_2d_sincos_position_embedding(grid_size, embed_dim, num_tokens=1, temperature=10000.):
    ...
    Args:
        grid_size: Grid size (H, W) or single int
        embed_dim: Embedding dimension (must be divisible by 4 for 2D sin-cos)
        num_tokens: Number of extra tokens (e.g., 1 for CLS token)
        temperature: Temperature for sinusoidal encoding

    Returns:
        Position embedding of shape [1, num_tokens + H*W, embed_dim]
    """
...
def build_perceptron_position_embedding(grid_size, embed_dim, num_tokens=1):
    """Build learned position embedding."""
    grid_size = to_2tuple(grid_size)
...
def patchify_image(x, patch_size):
    ...
    x = x.reshape(B, C, grid_size[0], patch_size[0], grid_size[1], patch_size[1])  # [B,C,gh,ph,gw,pw]
    x = x.permute(0, 2, 4, 1, 3, 5).reshape(B, np.prod(grid_size), C * np.prod(patch_size))  # [B,gh*gw,C*ph*pw]
...
class MAE2D(nn.Module):
    """2D Masked Autoencoder for OpenCell dataset"""
    def __init__(self,
                 encoder,
                 decoder,
                 args):
    ...
    def forward(self, x, return_image=False):
        ...
        x = patchify_image(x, self.patch_size)  # [B,gh*gw,C*ph*pw]
