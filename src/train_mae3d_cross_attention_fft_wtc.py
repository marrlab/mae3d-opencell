"""
Training script for MAE3D with Channel Cross-Attention and FFT Loss on WTC-11.

Trains a 3D Masked Autoencoder on WTC-11 single-cell volumes with:
- Channel-wise cross-attention between DNA and structure channels
- FFT loss on full per-channel reconstructions
- Input: (B, 2, 80, 224, 224)  [after centre-crop/pad]

Usage:
    # Single GPU
    python src/train_mae3d_cross_attention_fft_wtc.py \
...
from lib.trainers.mae3d_cross_attention_fft_wtc_trainer import (
    MAE3DChannelCrossAttentionFFTWTCTrainer,
)
...
def main():
    parser = argparse.ArgumentParser(description='MAE3D Cross-Attention + FFT Training on WTC-11')
    ...
    config_path = cmd_args.config if cmd_args.config is not None else \
        os.path.join(code_base_path, 'configs/wtc/wtc_3d_cross_attention_fft_kfold.yaml')
    ...
    if rank == 0:
        print(f'\n{"="*60}')
        print(f'MAE3D Cross-Attention + FFT — WTC-11')
    ...
    trainer = MAE3DChannelCrossAttentionFFTWTCTrainer(args)
    trainer.build_model()
