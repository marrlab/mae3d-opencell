"""
Training script for MAE3D with Channel Cross-Attention and CLIP ESM2 on WTC-11.

Fine-tunes from a 3D FFT k-fold checkpoint with InfoNCE contrastive loss
between the image embedding and per-protein ESM2 sequence embedding.

Usage:
    # K-fold CLIP fine-tuning (fold k)
    python src/train_mae3d_cross_attention_clip_wtc.py \
        --config configs/wtc/wtc_3d_cross_attention_clip_kfold.yaml \
...
from lib.trainers.mae3d_cross_attention_clip_wtc_trainer import (
    MAE3DChannelCrossAttentionCLIPWTCTrainer,
)
...
def main():
    parser = argparse.ArgumentParser(description='MAE3D Cross-Attention + CLIP ESM2 Training on WTC-11')
    ...
    config_path = cmd_args.config if cmd_args.config is not None else \
        os.path.join(code_base_path, 'configs/wtc/wtc_3d_cross_attention_clip_kfold.yaml')
    ...
    if rank == 0:
        print(f'\n{"="*60}')
        print(f'MAE3D Cross-Attention + CLIP ESM2 — WTC-11')
        print(f'Input: (B, 2, 80, 224, 224)')
        print(f'Mask ratio: {args.mask_ratio}')
        print(f'CLIP embed dim: {getattr(args, "clip_embed_dim", 768)}')
        print(f'CLIP weight: {getattr(args, "clip_weight", 0.5)}')
        if cmd_args.fold is not None:
            print(f'K-Fold: fold {cmd_args.fold}')
        print(f'{"="*60}\n')

    trainer = MAE3DChannelCrossAttentionCLIPWTCTrainer(args)
    trainer.build_model()
