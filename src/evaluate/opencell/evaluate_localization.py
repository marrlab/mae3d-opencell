...
def evaluate_model(model, dataloader, device, use_embedding_mode=False):
    ...
    print(f"Running evaluation (mode: {'embedding' if use_embedding_mode else 'image'})...")
    with torch.no_grad():
...
def main():
    ...
    # Evaluate
    logits, targets = evaluate_model(model, dataloader, device, use_embedding_mode=use_embedding_mode)
