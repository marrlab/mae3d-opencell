#!/usr/bin/env python3
"""
Training script for PPI prediction using SubCell embeddings via metric learning.

This script trains a lightweight projection head on top of precomputed SubCell
embeddings for protein-protein interaction prediction.

Usage:
    python src/train_ppi_subcell.py configs/opencell/opencell_ppi_subcell.yaml
"""
...
from lib.trainers.ppi_subcell_trainer import PPISubCellTrainer
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train PPI SubCell Model')
    parser.add_argument('config', type=str, help='Path to config file')
    parser.add_argument('--opts', nargs=argparse.REMAINDER, default=None,
                       help='Override config options (e.g., --opts lr=1e-3)')
    ...
    if rank == 0:
        print("=" * 80)
        print("PPI SubCell Metric Learning Configuration:")
        print("=" * 80)
    ...
    # Create trainer
    trainer = PPISubCellTrainer(args)
